...
<title>Akuntansi Eureeka &mdash; Laporan Daftar Bank</title>
<?= $this->endSection(); ?>
  ...
  <div class="card">
    <div class="card-header">
      <div class="card-header-action">
        <a href="<?= base_url('laporanbankdaftar/printPDF?tglawal=' . $tglawal . '&tglakhir=' . $tglakhir . '&bank=' . $bank) ?>" class="btn btn-success" target="_blank">
          <i class="fas fa-print"></i> Cetak PDF
        </a>
      </div>
    </div>
    <div class="card-body">
      <form method="GET" action="<?= base_url('laporanbankdaftar') ?>">
        <div class="row">
          <div class="col-md-3">
          ...
          <div class="col-md-3">
            ...
            <select name="bank" class="form-control">
              ...
              <?php foreach ($dtbank as $bk): ?>
                <option value="<?= $bk->id_setupbank ?>" <?= $bk->id_setupbank == $bank ? 'selected' : '' ?>><?= $bk->nama_setupbank ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="col-md-3">
      ...
      <div class="table-responsive mt-5">
        <table class="table table-striped table-md display nowrap compact eureeka-table" id="myTable">
          ...
          <tbody>
            <!-- Iterasi Data -->
            <?php foreach ($dtkartu_bank as $key => $value) : ?>
              <tr>
                <td><?= $key + 1 ?></td>
