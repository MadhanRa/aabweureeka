...
<title>Akuntansi Eureeka &mdash; Laporan Daftar Hutang Per Nota</title>
<?= $this->endSection(); ?>
...
<section class="section">
  <div class="section-header">
    <h1>Daftar Hutang Supplier Per Nota</h1>
  </div>

  <div class="card">
    <div class="card-header">
      <div class="card-header-action">
        <a href="<?= base_url('laporandaftarhutangsuppliernota/printPDF?tglawal=' . $tglawal . '&tglakhir=' . $tglakhir) ?>" class="btn btn-success" target="_blank">
          <i class="fas fa-print"></i> Cetak PDF
        </a>
      </div>
    </div>
    <div class="card-body">
      <form method="GET" action="<?= base_url('laporandaftarhutangsuppliernota') ?>">
        <div class="row">
          <div class="col-md-2">
      ...
      <div class="table-responsive mt-5">
        <table class="table table-striped table-md display nowrap compact eureeka-table" id="myTable">
          ...
          <tbody>
            <!-- Iterasi Data -->
            <?php foreach ($dtdaftar_hutang as $key => $value) : ?>
              <tr>
                <td><?= $key + 1 ?></td>
                <td><?= $value->kode ?></td>
                <td><?= $value->nama ?></td>
