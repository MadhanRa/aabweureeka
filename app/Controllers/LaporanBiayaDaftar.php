...
class LaporanBiayaDaftar extends BaseController
{
    ...
    public function index()
    {
        ...
        $daftar_biaya = $this->modelBiayaTransaksi->getDaftarBiaya($tglawal, $tglakhir, $rekening);
        ...
        foreach ($daftar_biaya as $row) {
            $awal_debet += isset($row->debit_saldo_awal) ? floatval($row->debit_saldo_awal) : 0;
            $debit_total += floatval($row->debit_biasa);
            $kredit_total += floatval($row->kredit);
            $akhir_debet +=  $row->debit_saldo_awal + $row->debit_biasa - $row->kredit;
        }

        // Ambil data tambahan untuk dropdown filter
        $data = [
            'dtdaftar_biaya'    => $daftar_biaya,
            'awal_debet'      => $awal_debet,
        ...
        return view('laporan_keuangan/laporan_biaya_daftar/index', $data);
    }
