...
class LaporanBankKartu extends BaseController
{
    ...
    public function index()
    {
        ...
        $bank = $this->request->getVar('bank') ? $this->request->getVar('bank') : '';

        // Panggil model untuk mendapatkan data laporan
        $riwayat_transaksi_bank = $this->objRiwayatTransaksi->get_laporan($tglawal, $tglakhir, $bank);
        $riwayat_tb_summary = $this->objRiwayatTransaksi->get_laporan_summary($tglawal, $tglakhir, $bank);
        ...
        foreach ($riwayat_tb_summary as $row) {
            $saldo_awal_total += floatval($row->saldo_awal);
            $debit_total += floatval($row->debit);
            $kredit_total += floatval($row->kredit);
            $saldo_akhir_total +=  floatval($row->saldo_berjalan);
        }

        // Ambil data tambahan untuk dropdown filter
        $data = [
            'dtkartu_bank'    => $riwayat_transaksi_bank,
            'saldo_awal_total'      => $saldo_awal_total,
            'debit_total'       => $debit_total,
            'kredit_total'  => $kredit_total,
            'saldo_akhir_total'       => $saldo_akhir_total,
            'dtbank'     => $this->objSetupBank->findAll(),
            'tglawal'        => $tglawal,
            'tglakhir'       => $tglakhir,
            'bank'       => $bank,
        ];

        return view('laporan_bank_kartu/index', $data);
    }
