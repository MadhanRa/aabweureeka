...
class ModelMutasiStock extends Model
{
    ...
    public function get_laporan_daftar_kosong($tglawal, $tglakhir, $kelompok, $group, $lokasi, $supplier)
    {
        // First get stock information
        $stockBuilder = $this->db->table('stock1 AS s');
        $stockBuilder->select('
...
');
        ...
        foreach ($stocks as $stock) {
            ...
            // Only add this stock to the result if both qty1 and qty2 are zero (empty)
            if ($stock->ending_qty1 == 0 && $stock->ending_qty2 == 0) {
                $emptyStocks[] = $stock;
            }
        }
    ...
    public function get_laporan_daftar_summary_kosong($tglawal, $tglakhir, $kelompok, $group, $lokasi, $supplier)
    {
        // First get the list of empty stocks (reusing the get_laporan_daftar_kosong method logic)
        $emptyStocks = $this->get_laporan_daftar_kosong($tglawal, $tglakhir, $kelompok, $group, $lokasi, $supplier);

        // Extract just the stock IDs from the empty stocks
        $emptyStockIds = array_map(function ($item) {
            return $item->id_stock;
        }, $emptyStocks);

        // If no empty stocks match filters, return empty summary with zeros
        if (empty($emptyStockIds)) {
            $summary = new \stdClass();
            $summary->initial_nilai = 0;
            $summary->in_nilai = 0;
            $summary->out_nilai = 0;
        ...
        SUM(CASE WHEN jenis = "masuk" THEN nilai ELSE -nilai END) as initial_nilai
    ');
        $initialBuilder->whereIn('id_stock', $emptyStockIds);
        ...
        $inBuilder = $this->db->table('mutasi_stock');
        $inBuilder->select('SUM(nilai) as in_nilai');
        $inBuilder->whereIn('id_stock', $emptyStockIds);
        $inBuilder->where('jenis', 'masuk');
        ...
        $outBuilder = $this->db->table('mutasi_stock');
        $outBuilder->select('SUM(nilai) as out_nilai');
        $outBuilder->whereIn('id_stock', $emptyStockIds);
        $outBuilder->where('jenis', 'keluar');
    ...
    public function get_laporan_daftar_total_kosong($kelompok, $group, $lokasi, $supplier)
    {
        // First get the list of empty stocks
        $emptyStocks = $this->get_laporan_daftar_kosong(null, null, $kelompok, $group, $lokasi, $supplier);
        ...
        // If no empty stocks match filters, return zeros
        if (empty($emptyStockIds)) {
            $totals = new \stdClass();
            $totals->total_awal = 0;
            $totals->total_masuk = 0;
        ...
        // Calculate in and out values for filtered empty stock IDs
        $builder = $this->db->table('mutasi_stock');
        $builder->select('
    SUM(CASE WHEN jenis = "masuk" THEN nilai ELSE 0 END) as total_masuk,
    SUM(CASE WHEN jenis = "keluar" THEN nilai ELSE 0 END) as total_keluar
');
        $builder->whereIn('id_stock', $emptyStockIds);
    ...
    public function get_laporan_daftar_minimal($tglawal, $tglakhir, $kelompok, $group, $lokasi, $supplier)
    {
        // First get stock information
        $stockBuilder = $this->db->table('stock1 AS s');
        $stockBuilder->select('
    ...
    ');
        ...
        // Only consider stocks that have a minimum value set
        $stockBuilder->where('s.min_stock >', 0);
        ...
        ');
            ...
            // Calculate average price
            $stock->normalized_qty = $stock->ending_qty1 + ($stock->ending_qty2 / $stock->conv_factor);

            $stock->converted_qty = $stock->ending_qty1 * $stock->conv_factor + $stock->ending_qty2;

            if ($stock->normalized_qty > 0) {
                $stock->rata_rata = $stock->ending_nilai / $stock->normalized_qty;
            } else {
                $stock->rata_rata = 0;
            }

            // Only add this stock to the result if the normalized quantity is less than or equal to min_stock
            if ($stock->converted_qty <= $stock->min_stock && $stock->converted_qty > 0) {
                $minimalStocks[] = $stock;
            }
        }

        return $minimalStocks;
    }

    public function get_laporan_daftar_summary_minimal($tglawal, $tglakhir, $kelompok, $group, $lokasi, $supplier)
    {
        // First get the list of minimal stocks (reusing the get_laporan_daftar_minimal method logic)
        $minimalStocks = $this->get_laporan_daftar_minimal($tglawal, $tglakhir, $kelompok, $group, $lokasi, $supplier);

        // Extract just the stock IDs from the minimal stocks
        $minimalStockIds = array_map(function ($item) {
            return $item->id_stock;
        }, $minimalStocks);

        // If no minimal stocks match filters, return empty summary with zeros
        if (empty($minimalStockIds)) {
            $summary = new \stdClass();
            $summary->initial_nilai = 0;
            $summary->in_nilai = 0;
            $summary->out_nilai = 0;
        ...
        SUM(CASE WHEN jenis = "masuk" THEN nilai ELSE -nilai END) as initial_nilai
    ');
        $initialBuilder->whereIn('id_stock', $minimalStockIds);
        ...
        $inBuilder = $this->db->table('mutasi_stock');
        $inBuilder->select('SUM(nilai) as in_nilai');
        $inBuilder->whereIn('id_stock', $minimalStockIds);
        $inBuilder->where('jenis', 'masuk');
        ...
        $outBuilder = $this->db->table('mutasi_stock');
        $outBuilder->select('SUM(nilai) as out_nilai');
        $outBuilder->whereIn('id_stock', $minimalStockIds);
        $outBuilder->where('jenis', 'keluar');
    ...
    public function get_laporan_daftar_total_minimal($kelompok, $group, $lokasi, $supplier)
    {
        // First get the list of minimal stocks (NOT empty stocks)
        $minimalStocks = $this->get_laporan_daftar_minimal(null, null, $kelompok, $group, $lokasi, $supplier);
        ...
        // If no minimal stocks match filters, return zeros
        if (empty($minimalStockIds)) {
            $totals = new \stdClass();
            $totals->total_awal = 0;
            $totals->total_masuk = 0;
        ...
        // Calculate in and out values for filtered minimal stock IDs
        $builder = $this->db->table('mutasi_stock');
        $builder->select('
        SUM(CASE WHEN jenis = "masuk" THEN nilai ELSE 0 END) as total_masuk,
        SUM(CASE WHEN jenis = "keluar" THEN nilai ELSE 0 END) as total_keluar
    ');
        $builder->whereIn('id_stock', $minimalStockIds);
