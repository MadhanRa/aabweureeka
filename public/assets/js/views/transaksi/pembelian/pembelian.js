// Document ready handler : Inisasi form components and event handlers
...
function initPpnOptionHandling() {
    ...
    $('#ppn').on('change keyup', handlePPNInputToggle);
}
...
function initializeDateHandling() {
    ...
    function updateJatuhTempo() {
        ...
        try {
            ...
            jatuhTempoInput.value = formatDate
        } catch (error) {
...
function setupTunaiInput() {
    ...
    $(SELECTORS.tunai).on('change blur', function () {
...
function computeDiscount(row, num, base) {
    ...
    const rp = parseFloat(rpField.attr('data-raw-value')) || 0;

    // Reset disabled status first
    row.find(`input[name$="[disc_${num}_rp]"]`).prop('disabled', false);
    row.find(`input[name$="[disc_${num}_perc]"]`).prop('disabled', false);

    if (perc > 0) {
        row.find(`input[name$="[disc_${num}_rp]"]`).prop('disabled', true);
        return (perc / 100) * base;
    } else if (rp > 0) {
        row.find(`input[name$="[disc_${num}_perc]"]`).prop('disabled', true);
        return rp;
    }
    return 0;
}
...
function formatNumericFields(row) {
    ...
    ['[jml_harga]', '[disc_1_rp]',
        ...
        let val = field.val() || 0;
        if (val.includes('Rp')) val = parseCurrencyValue(val);
        // Store the raw value as a data attribute
        field.attr('data-raw-value', val);
        // Display formatted value
        field.val(formatCurrency(val));
    });
}
...
function hitung_hutang() {
    ...
    $('#hutang').val(formatCurrency(total - tunai));
}
...
function fillAutoCompleteFields(row, item) {
    ...
    row.find('input[name$="[jml_harga]"]').val(0);
